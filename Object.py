# -*- coding: utf-8 -*-
"""
Created on Tue May 19 10:40:12 2020
...
# -*- coding: utf-8 -*-
"""
Created on Sat May 16 10:51:59 2020
...
def create_sequences(tok, tokenizer, max_length, max_length_obj, descriptions, photos, objects):
    X1, X2, X3, y = list(), list(), list(), list()
    for key, desc_list in objects.items():
        #print(key,desc_list)
        for desc in desc_list:
            seq = tokenizer.texts_to_sequences([desc])[0]
            seq_obj = tok.texts_to_sequences([desc_list[0][0]])[0]
            #seq_obj2 = tokenizer.texts_to_sequences([desc_list[0][1]])[0]
            #print(len(desc))
            for i in range(2, len(seq)):
                obj_seq = pad_sequences([seq_obj], maxlen=max_length_obj)[0]
                #obj_seq = to_categorical([obj_seq], num_classes=vocab)[0]
                #scene_seq = pad_sequences([seq_obj2], maxlen=max_length_obj)[0]
                #print("dsf",obj_seq)
...
def define_model(vocab_size, max_length, max_length_obj):
    ...
    #in1 = Embedding(vocab, 256, mask_zero=True)(inputs2)
...
print("Objects Vocabulary:%d" % vocab)#114
vocab_size = len(tokenizer.word_index) + 1
print('Vocabulary Size: %d' % vocab_size)#7579

max_length = max_length(train_descriptions)
print('Description Length: %d' % max_length)#34

max_length_obj = max_length_Obj(file)
print('Objects Length: %d' % max_length_obj)#49

X1train, X2train, X3train, ytrain = create_sequences(tok, tokenizer, max_length, max_length_obj, train_descriptions, train_features, objects)
...
objects_test = load_object('Objects_test.txt', test)
print('Objects: test=',len(objects_test))
 
test_features = load_photo_features('features.pkl', test)
print('Photos: test=%d' % len(test_features))

X1test, X2test, X3test, ytest = create_sequences(tok, tokenizer, max_length, max_length_obj, test_descriptions, test_features, objects_test)
...
model.fit([X1train, X2train, X3train], ytrain, epochs=11, verbose=2, callbacks=[checkpoint], validation_data=([X1test, X2test, X3test], ytest))
